# Claude Desktop Pro Instructions: dispatching-parallel-agents
...
2. **Activation:**
   ```
   "Use dispatching-parallel-agents skill to help me with..."
   ```

3. **Two Upload Methods:**

   **Method A: ZIP Upload (Recommended)**
   - Pre-built ZIP available in `skill-zips/dispatching-parallel-agents.zip`
   ...
   **Method B: Project Knowledge**
   ...
   - Upload `dispatching-parallel-agents.md` to project knowledge
...
1. **Explicit Invocation:**
   ...
   ```
   "Use the dispatching-parallel-agents skill for this task"
   ```
...
3. **Manual Tracking (without TodoWrite):**
   ...
   ```
   "Track the dispatching-parallel-agents checklist as we go"
   ```

4. **Verification:**
   ```
   "Verify all dispatching-parallel-agents requirements are met"
   ```
...
Claude: "I'm using the dispatching-parallel-agents skill to approach this systematically.
...
```
"Use brainstorming to design, then dispatching-parallel-agents to implement"
```

### ZIP Upload Details

**Location:** `superpowers-desktop/pro-setup/skill-zips/dispatching-parallel-agents.zip`

**Contents:**
- Main skill file: `dispatching-parallel-agents.md`
...
3. Upload `dispatching-parallel-agents.md`
...
1. **Use TodoWrite for tracking:**
   ```
   "Create a TodoWrite checklist for dispatching-parallel-agents steps"
   ```
...
- Try: "Check your capabilities for dispatching-parallel-agents"
...
- **Source:** `superpowers-desktop/pro-setup/skills/collaboration/dispatching-parallel-agents.md`
- **ZIP:** `superpowers-desktop/pro-setup/skill-zips/dispatching-parallel-agents.zip`
