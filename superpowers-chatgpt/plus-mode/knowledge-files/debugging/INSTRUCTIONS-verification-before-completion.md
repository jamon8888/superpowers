# ChatGPT Pro Instructions: verification-before-completion
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from verification-before-completion in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all verification-before-completion requirements"
   ```
...
You: "I'm going to use the verification-before-completion skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then verification-before-completion to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use verification-before-completion for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all verification-before-completion requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check verification-before-completion.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for verification-before-completion"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/debugging/verification-before-completion.md`
