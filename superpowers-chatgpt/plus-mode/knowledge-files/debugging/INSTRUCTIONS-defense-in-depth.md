# ChatGPT Pro Instructions: defense-in-depth
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from defense-in-depth in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all defense-in-depth requirements"
   ```
...
You: "I'm going to use the defense-in-depth skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then defense-in-depth to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use defense-in-depth for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all defense-in-depth requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check defense-in-depth.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for defense-in-depth"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/debugging/defense-in-depth.md`
