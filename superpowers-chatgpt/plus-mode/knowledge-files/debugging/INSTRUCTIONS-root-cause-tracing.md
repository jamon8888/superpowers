# ChatGPT Pro Instructions: root-cause-tracing
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from root-cause-tracing in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all root-cause-tracing requirements"
   ```
...
You: "I'm going to use the root-cause-tracing skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then root-cause-tracing to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use root-cause-tracing for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all root-cause-tracing requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check root-cause-tracing.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for root-cause-tracing"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/debugging/root-cause-tracing.md`
