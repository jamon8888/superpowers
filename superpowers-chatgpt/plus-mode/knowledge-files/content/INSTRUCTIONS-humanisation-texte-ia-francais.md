# ChatGPT Pro Instructions: humanisation-texte-ia-francais
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from humanisation-texte-ia-francais in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all humanisation-texte-ia-francais requirements"
   ```
...
You: "I'm going to use the humanisation-texte-ia-francais skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then humanisation-texte-ia-francais to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use humanisation-texte-ia-francais for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all humanisation-texte-ia-francais requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check humanisation-texte-ia-francais.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for humanisation-texte-ia-francais"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/content/humanisation-texte-ia-francais.md`
