# ChatGPT Pro Instructions: ux-writing
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from ux-writing in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all ux-writing requirements"
   ```
...
You: "I'm going to use the ux-writing skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then ux-writing to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use ux-writing for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all ux-writing requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check ux-writing.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for ux-writing"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/content/ux-writing.md`
