# ChatGPT Pro Instructions: voice-chart-template
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from voice-chart-template in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all voice-chart-template requirements"
   ```
...
You: "I'm going to use the voice-chart-template skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then voice-chart-template to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use voice-chart-template for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all voice-chart-template requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check voice-chart-template.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for voice-chart-template"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/content/voice-chart-template.md`
