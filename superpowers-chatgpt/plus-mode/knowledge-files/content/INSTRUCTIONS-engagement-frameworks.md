# ChatGPT Pro Instructions: engagement-frameworks
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from engagement-frameworks in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all engagement-frameworks requirements"
   ```
...
You: "I'm going to use the engagement-frameworks skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then engagement-frameworks to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use engagement-frameworks for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all engagement-frameworks requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check engagement-frameworks.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for engagement-frameworks"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/content/engagement-frameworks.md`
