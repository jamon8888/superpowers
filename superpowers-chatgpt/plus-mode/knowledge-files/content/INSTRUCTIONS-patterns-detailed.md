# ChatGPT Pro Instructions: patterns-detailed
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from patterns-detailed in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all patterns-detailed requirements"
   ```
...
You: "I'm going to use the patterns-detailed skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then patterns-detailed to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use patterns-detailed for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all patterns-detailed requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check patterns-detailed.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for patterns-detailed"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/content/patterns-detailed.md`
