# ChatGPT Pro Instructions: error-message-template
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from error-message-template in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all error-message-template requirements"
   ```
...
You: "I'm going to use the error-message-template skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then error-message-template to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use error-message-template for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all error-message-template requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check error-message-template.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for error-message-template"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/content/error-message-template.md`
