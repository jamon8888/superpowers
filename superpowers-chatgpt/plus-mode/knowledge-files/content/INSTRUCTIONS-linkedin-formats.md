# ChatGPT Pro Instructions: linkedin-formats
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from linkedin-formats in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all linkedin-formats requirements"
   ```
...
You: "I'm going to use the linkedin-formats skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then linkedin-formats to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use linkedin-formats for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all linkedin-formats requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check linkedin-formats.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for linkedin-formats"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/content/linkedin-formats.md`
