# ChatGPT Pro Instructions: ai-text-humanization
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from ai-text-humanization in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all ai-text-humanization requirements"
   ```
...
You: "I'm going to use the ai-text-humanization skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then ai-text-humanization to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use ai-text-humanization for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all ai-text-humanization requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check ai-text-humanization.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for ai-text-humanization"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/content/ai-text-humanization.md`
