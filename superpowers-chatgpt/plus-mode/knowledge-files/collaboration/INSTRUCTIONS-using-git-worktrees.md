# ChatGPT Pro Instructions: using-git-worktrees
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from using-git-worktrees in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all using-git-worktrees requirements"
   ```
...
You: "I'm going to use the using-git-worktrees skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then using-git-worktrees to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use using-git-worktrees for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all using-git-worktrees requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check using-git-worktrees.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for using-git-worktrees"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/collaboration/using-git-worktrees.md`
