# ChatGPT Pro Instructions: dispatching-parallel-agents
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from dispatching-parallel-agents in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all dispatching-parallel-agents requirements"
   ```
...
You: "I'm going to use the dispatching-parallel-agents skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then dispatching-parallel-agents to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use dispatching-parallel-agents for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all dispatching-parallel-agents requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check dispatching-parallel-agents.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for dispatching-parallel-agents"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/collaboration/dispatching-parallel-agents.md`
