# ChatGPT Pro Instructions: requesting-code-review
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from requesting-code-review in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all requesting-code-review requirements"
   ```
...
You: "I'm going to use the requesting-code-review skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then requesting-code-review to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use requesting-code-review for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all requesting-code-review requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check requesting-code-review.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for requesting-code-review"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/collaboration/requesting-code-review.md`
