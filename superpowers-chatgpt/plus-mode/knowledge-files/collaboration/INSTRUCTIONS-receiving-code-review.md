# ChatGPT Pro Instructions: receiving-code-review
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from receiving-code-review in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all receiving-code-review requirements"
   ```
...
You: "I'm going to use the receiving-code-review skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then receiving-code-review to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use receiving-code-review for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all receiving-code-review requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check receiving-code-review.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for receiving-code-review"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/collaboration/receiving-code-review.md`
