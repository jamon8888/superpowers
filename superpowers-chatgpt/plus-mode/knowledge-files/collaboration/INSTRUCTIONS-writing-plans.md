# ChatGPT Pro Instructions: writing-plans
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from writing-plans in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all writing-plans requirements"
   ```
...
You: "I'm going to use the writing-plans skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then writing-plans to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use writing-plans for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all writing-plans requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check writing-plans.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for writing-plans"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/collaboration/writing-plans.md`
