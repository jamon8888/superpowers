# ChatGPT Pro Instructions: executing-plans
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from executing-plans in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all executing-plans requirements"
   ```
...
You: "I'm going to use the executing-plans skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then executing-plans to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use executing-plans for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all executing-plans requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check executing-plans.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for executing-plans"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/collaboration/executing-plans.md`
