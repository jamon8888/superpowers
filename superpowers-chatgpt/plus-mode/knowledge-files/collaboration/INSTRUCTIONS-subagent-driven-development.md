# ChatGPT Pro Instructions: subagent-driven-development
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from subagent-driven-development in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all subagent-driven-development requirements"
   ```
...
You: "I'm going to use the subagent-driven-development skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then subagent-driven-development to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use subagent-driven-development for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all subagent-driven-development requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check subagent-driven-development.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for subagent-driven-development"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/collaboration/subagent-driven-development.md`
