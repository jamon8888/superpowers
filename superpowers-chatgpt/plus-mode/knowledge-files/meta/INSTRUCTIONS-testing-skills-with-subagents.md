# ChatGPT Pro Instructions: testing-skills-with-subagents
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from testing-skills-with-subagents in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all testing-skills-with-subagents requirements"
   ```
...
You: "I'm going to use the testing-skills-with-subagents skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then testing-skills-with-subagents to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use testing-skills-with-subagents for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all testing-skills-with-subagents requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check testing-skills-with-subagents.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for testing-skills-with-subagents"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/meta/testing-skills-with-subagents.md`
