# ChatGPT Pro Instructions: sharing-skills
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from sharing-skills in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all sharing-skills requirements"
   ```
...
You: "I'm going to use the sharing-skills skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then sharing-skills to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use sharing-skills for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all sharing-skills requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check sharing-skills.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for sharing-skills"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/meta/sharing-skills.md`
