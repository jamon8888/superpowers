# ChatGPT Pro Instructions: systematic-debugging
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from systematic-debugging in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all systematic-debugging requirements"
   ```
...
You: "I'm going to use the systematic-debugging skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then systematic-debugging to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use systematic-debugging for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all systematic-debugging requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check systematic-debugging.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for systematic-debugging"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/core/systematic-debugging.md`
