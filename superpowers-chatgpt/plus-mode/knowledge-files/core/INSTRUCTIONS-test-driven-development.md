# ChatGPT Pro Instructions: test-driven-development
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from test-driven-development in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all test-driven-development requirements"
   ```
...
You: "I'm going to use the test-driven-development skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then test-driven-development to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use test-driven-development for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all test-driven-development requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check test-driven-development.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for test-driven-development"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/core/test-driven-development.md`
