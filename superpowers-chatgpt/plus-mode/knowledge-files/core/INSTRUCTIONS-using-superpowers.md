# ChatGPT Pro Instructions: using-superpowers
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from using-superpowers in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all using-superpowers requirements"
   ```
...
You: "I'm going to use the using-superpowers skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then using-superpowers to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use using-superpowers for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all using-superpowers requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check using-superpowers.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for using-superpowers"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/core/using-superpowers.md`
