# ChatGPT Pro Instructions: brainstorming
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from brainstorming in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all brainstorming requirements"
   ```
...
You: "I'm going to use the brainstorming skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then brainstorming to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use brainstorming for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all brainstorming requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check brainstorming.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for brainstorming"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/core/brainstorming.md`
