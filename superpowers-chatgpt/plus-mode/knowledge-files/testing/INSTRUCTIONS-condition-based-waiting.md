# ChatGPT Pro Instructions: condition-based-waiting
...
2. **Manual Tracking:**
   ...
   ```
   "Track the checklist from condition-based-waiting in your response"
   ```

3. **Verification:**
   After each phase:
   ```
   "Verify that we've completed all condition-based-waiting requirements"
   ```
...
You: "I'm going to use the condition-based-waiting skill to approach this systematically.
...
```
"Use brainstorming skill to design this, then condition-based-waiting to implement"
```
...
1. **Start conversations with skill invocation:**
   ```
   "Let's use condition-based-waiting for this task"
   ```
...
3. **Verify completion:**
   ```
   "Did we follow all condition-based-waiting requirements?"
   ```

4. **Reference project knowledge:**
   ```
   "Check condition-based-waiting.md for the exact steps"
   ```
...
- Try: "Check your knowledge files for condition-based-waiting"
...
- **Source:** `superpowers-chatgpt/plus-mode/knowledge-files/testing/condition-based-waiting.md`
